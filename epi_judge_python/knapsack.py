...
def optimum_subject_to_capacity(items, capacity):
    cache = [[0 for _ in range(capacity + 1)] for _ in range(len(items))]

    # items.sort(key=lambda it: it[0])

    for j in range(capacity + 1):
        cache[0][j] = items[0][1] if j >= items[0][0] else 0

    for i in range(1, len(items)):
        for j in range(capacity + 1):
            curr_item = items[i]
            if j >= curr_item[0]:
                cache[i][j] = max(cache[i - 1][j], cache[i - 1][j - curr_item[0]] + curr_item[1])
            else:
